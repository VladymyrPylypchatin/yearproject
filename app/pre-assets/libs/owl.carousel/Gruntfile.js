		...
		.initConfig({
			...
			assemble: {
				options: {
					...
					expand: true,
					production: false,
					assets: '<%= app.docs.dest %>/pre-assets',
			...
			sass: {
				docs: {
					options: {
						...
						includePaths: [ '<%= app.docs.src %>/pre-assets/scss/', 'node_modules/foundation-sites/scss' ]
					},
					files: {
						'<%= app.docs.dest %>/assets/css/docs.theme.min.css': '<%= app.docs.src %>/pre-assets/scss/docs.theme.scss'
					}
				},
				dist: {
					options: {
			...
			autoprefixer: {
				...
				dist: {
					files: {
						'dist/assets/<%= pkg.name %>.css': 'dist/pre-assets/<%= pkg.name %>.css',
						'dist/assets/owl.theme.default.css': 'dist/pre-assets/owl.theme.default.css',
						'dist/assets/owl.theme.green.css': 'dist/pre-assets/owl.theme.green.css'
					}
				}
			},
			...
			cssmin: {
				dist: {
					files: {
						'dist/assets/<%= pkg.name %>.min.css': 'dist/pre-assets/<%= pkg.name %>.css',
						'dist/assets/owl.theme.default.min.css': 'dist/pre-assets/owl.theme.default.css',
						'dist/assets/owl.theme.green.min.css': 'dist/pre-assets/owl.theme.green.css'
					}
				}
			},
			...
			copy: {
				...
				distToDocs: {
					...
					cwd: 'dist/',
					src: [ '**/*.*' ],
					dest: '<%= app.docs.dest %>/pre-assets/owlcarousel'
				},
				...
				srcToDocs: {
					...
					cwd: 'src/js',
					src: [ '**/*.js' ],
					dest: '<%= app.docs.dest %>/pre-assets/owlcarousel/src'
				},

				docsAssets: {
					expand: true,
					cwd: '<%= app.docs.src %>/pre-assets/',
					src: [ 'css/*.css', 'vendors/*.js', 'vendors/*.map', 'img/*.*', 'js/*.*' ],
					dest: '<%= app.docs.dest %>/pre-assets/'
				},
			...
			watch: {
				...
				sassDocs: {
					files: [ '<%= app.docs.src %>/pre-assets/**/*.scss' ],
					tasks: [ 'sass:docs' ]
				},
				sass: {
					files: [ 'src/**/*.scss' ],
				...
				jsDocs: {
					files: [ '<%= app.docs.src %>/pre-assets/**/*.js' ],
